#!/usr/bin/env python3
"""Build one hash-closed RoMa proposal shard from a public tensor cache."""
...
from p2g.training.roma_point_provider import build_roma_point_proposals
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build one append-only pixel-to-3D proposal shard from a public "
            "p2g.tensor_cache.v1 cache. Model weights remain external and are "
    ...
    parser.add_argument("--frame-id", type=_nonnegative_integer, required=True)
    parser.add_argument(
        "--roma-indoor-weight",
        type=_path,
        required=True,
    ...
    parser.add_argument(
        ...
        type=_path,
        required=True,
        help="new append-only output directory; an existing path is rejected",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        # Library/provider progress belongs on stderr; stdout stays one JSON value.
        ...
        print(f"RoMa proposal build failed: {exc}", file=sys.stderr)
        return 2
